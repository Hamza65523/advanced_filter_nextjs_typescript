...
const index:NextPage<Props> = ({priceRange,changePrice,changedCheckId,cuision,selectedCategory,handleSlectCategory,selectedRating,handlerselectedRating}) => {  
  return (
    <div className='border-2 border-gray-50 px-2 rounded-md  py-2 shadow-xl '>
      <div className="w-72 px-2 py-4">
        ...
        <FilterToggle value={selectedCategory} selectToggle={handleSlectCategory} options={categoriesList}/>
        <h4 className='font-bold py-2 text-xl'>Cuisine</h4>
        {cuision.map((items:any)=>(
        <CheckBoxPortion key={items.id} cuision={items} changedCheckId={changedCheckId}/>
        ))}
        ...
        <FilterToggle value={selectedRating} selectToggle={handlerselectedRating} options={ratingList}/>
        </div>
      </div>
    </div>
  )
